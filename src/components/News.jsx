...
export class News extends Component {
  ...
  Scroll(event) {
    ...
    if (scrollTop + clientHeight >= scrollHeight) {
      if (
        ...
        ((this.state.totalResults % this.props.pageSize === 0 &&
          this.state.page <=
            Math.floor(this.state.totalResults / this.props.pageSize)) ||
          this.state.page <=
            1 + Math.floor(this.state.totalResults / this.props.pageSize))
      ) {
        this.update_News();
  ...
  async componentDidMount() {
    //runs after rendering through render()
  ...
  componentWillUnmount() {
    //runs before rendering through render()
          ...
          {!this.state.loading && (
            <Stack>
              ...
              <Grid
                container
                spacing={4}
                sx={{ justifyContent: { sm: "initial", xs: "center" } }}
              >
                  ...
                  return (
                    <Grid item md={4} sm={6} key={index}>
                      <NewsItem
                        ...
                        description={element?.description?.slice(0, 500)}
                        newsUrl={element?.url}
